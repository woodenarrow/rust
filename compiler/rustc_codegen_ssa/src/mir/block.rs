...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    pub fn codegen_block(&mut self, bb: mir::BasicBlock) {
        ...
        let optimised_terminator;
        if let Some((enum_ty, discr, switch_ty, targets)) =
            self.get_discriminant_switch_block_parts(statements, terminator)
        {
            let span = debug_span!("opt_switch_monomorphizing");
            let _enter = span.enter();

            debug!("found switch terminator on {} discriminant", enum_ty);
            ...
            macro_rules! optimize_terminator {
                ($kind:expr) => {
                    optimised_terminator =
                        mir::Terminator { source_info: terminator.source_info, kind: $kind };
                    terminator = &optimised_terminator;
                };
            }

            match enum_layout.layout.variants {
                _ if enum_layout.abi.is_uninhabited() => {
                    debug!("optimized switch on uninhabited type to unreachable");
                ...
                abi::Variants::Multiple { ref variants, ref tag_encoding, .. } => {
                    ...
                    let filtered_switch_targets = match tag_encoding {
                        abi::TagEncoding::Niche { .. } => filter_switch_targets(targets, |index| {
                        ...
                        abi::TagEncoding::Direct => {
                            ...
                            let discriminants = variants
                                ...
                                .collect::<Vec<_>>();
                            filter_switch_targets(targets, |discr| discriminants.contains(&discr))
                        }
                    };

                    if let Some(filtered_switch_targets) = filtered_switch_targets {
                        debug!(
                            "pruned discriminant switch arms (kept {} out of {} arms)",
                            filtered_switch_targets.iter().len(),
                            targets.iter().len(),
                        );
                        optimize_terminator!(mir::TerminatorKind::SwitchInt {
                            discr: discr.clone(),
                            switch_ty: switch_ty,
                            targets: filtered_switch_targets,
                        });
                    }
                }
            }
